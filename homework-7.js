...
const productName = "Антифриз 5л";
const productPrice = 1500;
const productPay = (budget) => {
  const budgetToNumber = Number(budget);
  if (!budgetToNumber || budget === true) {
    console.log("Некорректные входные данные");
    return;
  }
  if (budget >= productPrice) {
    ...
    const noMoney = productPrice - budget;
    console.log(`Вам не хватает ${noMoney}$, пополните баланс`);
  }
};
